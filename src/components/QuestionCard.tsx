            ...
            <button
              ...
              className={clsx(
                ...
                flaggedQuestions.includes(question.id)
                  ? (isDark ? "bg-orange-500/20 text-orange-300 border-orange-500/30 hover:bg-orange-500/30" : "bg-orange-500/20 text-orange-600 border-orange-500/30 hover:bg-orange-500/30")
                  : (isDark ? "text-glass-secondary border-transparent hover:bg-white/10 hover:text-glass-primary" : "text-glass-secondary border-transparent hover:bg-black/5 hover:text-glass-primary")
              )}
            >
        ...
        <div className="mb-8">
          ...
          {question.sectionId && config?.sections?.find(s => s.id === question.sectionId) && (
            (() => {
              ...
              return (
                <div className={`mb-6 p-5 glass-panel rounded-2xl ${isDark ? 'bg-indigo-900/10 border-indigo-500/20' : 'bg-indigo-50/50 border-indigo-100'}`}>
                  <h3 className={`text-xs font-bold uppercase tracking-widest mb-3 flex items-center gap-2 ${isDark ? 'text-indigo-400' : 'text-indigo-500'}`}>
