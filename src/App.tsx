    ...
    >
      ...
      <ErrorBoundary onReset={clearState}>
        {config ? (
          ...
          <div className="flex flex-col md:flex-row min-h-screen w-full pb-16 md:pb-0 relative">
            
            {/* Mobile Header (Only visible on small screens to house logo and settings) */}
            <div className={`md:hidden flex items-center justify-between px-4 py-3 border-b sticky top-0 z-40 ${isDark ? 'bg-[#0f111a]/90 backdrop-blur-md border-white/5' : 'bg-white/90 backdrop-blur-md border-indigo-100'}`}>
              <div className="flex items-center gap-2">
                <img src="/revise-logo.png" alt="Revise logo" className="w-8 h-8 rounded-lg" />
                <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Revise</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={toggleTheme}
                  className={`p-2 rounded-xl transition-all ${isDark ? 'text-slate-400 hover:bg-white/5 hover:text-slate-50' : 'text-slate-500 hover:bg-slate-100 hover:text-indigo-600'}`}
                >
                  {themeMode === 'light' ? <Moon className="w-5 h-5 text-indigo-600" /> : <Sun className="w-5 h-5 text-yellow-300" />}
                </button>
                <button
                  onClick={() => setIsSettingsOpen(true)}
                  className={`relative p-2 rounded-xl transition-all ${isDark ? 'text-slate-400 hover:bg-white/5 hover:text-slate-50' : 'text-slate-500 hover:bg-slate-100 hover:text-indigo-600'}`}
                >
            ...
            <div className={`
              fixed bottom-0 left-0 right-0 w-full h-16 z-50
              md:sticky md:top-0 md:w-24 md:h-screen md:bottom-auto
              border-t md:border-t-0 md:border-r 
              ${isDark ? 'border-white/5 bg-[#0f111a]' : 'border-indigo-100 bg-white/90 backdrop-blur-md'} 
              flex flex-row md:flex-col items-center py-0 md:py-6 shrink-0 transition-colors
            `}>
              <div className="hidden md:flex mb-8 w-12 h-12 md:w-14 md:h-14 items-center justify-center">
                {activeTab !== null && (
                  <motion.img
                    layoutId="main-logo"
                    src="/revise-logo.png"
              ...
              <nav className="flex flex-row md:flex-col gap-1 md:gap-4 w-full h-full md:h-auto px-1 md:px-3 items-center justify-around md:justify-start">
                <div className="w-full flex-1 md:flex-none">
                  <button
                    onClick={() => navigateToTab(null)}
                    className={`w-full flex flex-col items-center justify-center gap-1 py-2 md:py-3 rounded-xl transition-all ${activeTab === null
                      ? isDark
                        ? 'md:bg-indigo-500/20 text-indigo-300 md:shadow-sm md:border border-indigo-500/20'
                        : 'md:bg-indigo-50 text-indigo-600 md:shadow-sm md:border border-indigo-100'
                  ...
                  >
                    <Home className={`w-5 h-5 md:w-5 md:h-5 ${activeTab === null ? 'scale-110' : ''} transition-transform duration-300`} />
                ...
                {tabsConfig.map((tab) => {
                  ...
                  return (
                    <div key={tab.id} className="w-full flex-1 md:flex-none">
                      <button
                        onClick={() => navigateToTab(tab.id)}
                        className={`w-full flex flex-col items-center justify-center gap-1 py-2 md:py-3 rounded-xl transition-all ${isActive
                          ? isDark
                            ? 'md:bg-indigo-500/20 text-indigo-300 md:shadow-sm md:border border-indigo-500/20'
                            : 'md:bg-indigo-50 text-indigo-600 md:shadow-sm md:border border-indigo-100'
                          : isDark
                      ...
                      >
                        <tab.icon className={`w-5 h-5 md:w-5 md:h-5 ${isActive ? 'scale-110' : ''} transition-transform duration-300`} />
              ...
              <div className="mt-auto hidden md:flex flex-col gap-4 w-full px-2 md:px-3">
